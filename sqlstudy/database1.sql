...
"@@" işareti sistem değişkenlerine erişmek için kullanılır.
...
select @kitapsayi=count(kitapadi) from Kitaplar
select @kitapsayi
...
select count(KitapTür) from Kitaplar where KitapTür='roman'

**********************************************************************************************************************************************

declare @sayi tinyint
select @sayi=count(KitapTür) from Kitaplar where KitapTür='Roman'
if(@sayi<=3)
begin
...
select * from Kitaplar where Kitaprenk=@renk
...
select dbo.buyukharf(Kitaprengi) from Kitaplar
...
begin
declare @sayi int
select @sayi=count(KitapRengi) from Kitaplar where KitapRengi=@gelenrenk
return @sayi
end
...
returns table as 
return (select * from Kitaplar where KitapPuan>@gelenpuan)
